...
class AzureKeyVault(AzureResourceBase):
    ...
    def get_key(self, key_name: str) -> str:
        ...
        try:
            ...
            print(f"Error getting key {key_name}: {str(e)}")
            raise
    ...
    def list_keys(self) -> List[Dict]:
        ...
        Returns:
            List of dictionaries containing key properties
        """
        try:
            keys = []
    ...
    def create_key(self, key_name: str, key_type: str = "RSA", key_size: int = 2048) -> Dict:
        """
        Create a new key in the key vault.
        
        Args:
            key_name: Name of the key to create
            key_type: Type of key to create (default: "RSA")
            key_size: Size of the key in bits (default: 2048)
            
        Returns:
            Dictionary containing the created key properties
        """
        try:
            ...
            print(f"Error creating key {key_name}: {str(e)}")
            raise

    def get_certificate(self, certificate_name: str) -> Dict:
        """
        ...
        Returns:
            Dictionary containing the certificate properties
        """
        try:
            ...
            return {
                ...
                'updated_on': certificate.updated_on,
                'expires_on': certificate.expires_on,
                'enabled': certificate.properties.enabled,
                'subject': certificate.properties.subject,
                'issuer': certificate.properties.issuer
            }
        except Exception as e:
            print(f"Error getting certificate {certificate_name}: {str(e)}")
            raise
    ...
    def list_certificates(self) -> List[Dict]:
        ...
        Returns:
            List of dictionaries containing certificate properties
        """
        try:
            certificates = []
            for cert in self.certificate_client.list_properties_of_certificates():
                certificates.append({
                    'name': cert.name,
    ...
    def create_certificate(
        ...
        Args:
            certificate_name: Name of the certificate to create
            subject: Subject name for the certificate (e.g., "CN=example.com")
            validity_months: Number of months the certificate should be valid (default: 12)
            key_type: Type of key to use (default: "RSA")
            key_size: Size of the key in bits (default: 2048)
            
        Returns:
            Dictionary containing the created certificate properties
        """
        try:
            ...
            certificate = self.certificate_client.create_certificate(
                certificate_name=certificate_name,
                policy=policy
            )

            print(f"Successfully created certificate {certificate_name}")
            return {
                'name': certificate.name,
                'version': certificate.version,
                'created_on': certificate.created_on,
            ...
            print(f"Error creating certificate {certificate_name}: {str(e)}")
            raise

    def get_certificate_as_file(self, certificate_name: str, file_path: str = None) -> Tuple[str, str]:
        """
        Get a certificate from the key vault and save it as a file.
        
        Args:
            certificate_name: Name of the certificate to retrieve
            file_path: Optional path where to save the certificate. If not provided, a temporary file will be created.
            
        Returns:
            Tuple containing (certificate file path, password)
        """
        try:
            # Get the certificate
            certificate = self.certificate_client.get_certificate(certificate_name)
            ...
            if file_path is None:
                # Create a temporary file with .pfx extension
                temp_file = tempfile.NamedTemporaryFile(delete=False, suffix='.pfx')
                file_path = temp_file.name
                temp_file.close()  # Close the file handle to allow writing
            else:
            ...
            with open(file_path, 'wb') as f:
                f.write(certificate.cer)
            
            print(f"Certificate saved to file: {file_path}")
            return file_path, certificate.password
            
        except Exception as e:
            print(f"Error getting certificate {certificate_name} as file: {str(e)}")
            raise

    def set_certificate_from_file(
        self,
        certificate_name: str,
        ...
        """
        Set a certificate in the key vault from a file.
        
        Args:
            certificate_name: Name to give the certificate in the vault
            certificate_path: Path to the certificate file (.pfx or .pem)
            password: Password for the certificate file (if required)
        ...
        try:
            ...
            # Create the certificate
            certificate = self.certificate_client.import_certificate(
                certificate_name=certificate_name,
                certificate_bytes=certificate_data,
                password=password
            )
            
            print(f"Successfully imported certificate {certificate_name} from file")
            return {
                'name': certificate.name,
                'version': certificate.version,
                'created_on': certificate.created_on,
            ...
            print(f"Error importing certificate from file: {str(e)}")
            raise
        finally:
            # Clean up temporary file if it was created by get_certificate_as_file
            if certificate_path.startswith(tempfile.gettempdir()):
                try:
                    os.unlink(certificate_path)
                except Exception as e:
                    print(f"Warning: Could not delete temporary file {certificate_path}: {str(e)}")
